...
First, run Zookeeper on your machine, then you can access it and use as Service Registry and Configuration source with Spring Cloud Zookeeper.
...
We recommend using dependency management and `spring-cloud-starter-parent`.
...
When this HTTP server runs, it connects to Zookeeper running at the default local 2181 port.
...
The application will retrieve configuration data from Zookeeper.

=== Mixed Usage

If you are going to be using Spring Cloud Zookeper to retrieve both service discovery and configuration data from Zookeeper, the easiest way to use it is to include the `org.springframework.cloud:spring-cloud-starter-zookeeper-all` starter in your build setup.